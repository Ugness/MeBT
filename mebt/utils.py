...
def save_video_grid(video, fname, nrow=None, fps=10):
    b, c, t, h, w = video.shape
    video = video.permute(0, 2, 3, 4, 1)
    video = (video.cpu().numpy() * 255).astype('uint8')
    if nrow is None:
    ...
    video[0].save(fname, quality=95, save_all=True, append_images=video[1:], duration=1000/fps, loop=0, optimize=False)
    # skvideo.io.vwrite(fname, video_grid, inputdict={'-r': '5'})
    print('saved videos to', fname)
